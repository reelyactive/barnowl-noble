  ...
  handlePeripheral(peripheral, raddec, noble, callback) {
    ...
    peripheral.connect((error) => {
      ...
      let operation = { callback: callback, raddec: raddec,
                        ...
                        isActionComplete: Array(BLUVIB_DEFAULT_ACTIONS.length),
                        numberOfAxes: 1 };
      ...
      peripheral.discoverSomeServicesAndCharacteristics(BLUVIB_SERVICE_UUIDS,
        ...
        operation.isActionComplete.fill(false);
...
/**
 * Perform the concluding actions of an operation, and execute the operation's
 * callback once the peripheral is disconnected.
 * @param {Object} operation The operation parameters.
 */
function performConcludingActions(operation) {
  ...
  if(operation.receivedSampleBytes < operation.expectedSampleBytes) {
    console.log('barnowl-noble: BluVib incomplete sample (' +
                operation.receivedSampleBytes + ' of ' +
                operation.expectedSampleBytes + ' bytes received)');
  }

  if(operation.peripheral.state === 'connected') {
    operation.peripheral.once('disconnect', () => {
...
function performGattRead(characteristic, index, operation) {
  characteristic.read((error, data) => {
    ...
    // The Axes characteristic is only present on BluVib-M-x-3 sensors!
...
function performGattNotification(characteristic, index, operation) {
  ...
  operation.receivedSampleBytes = 0;

  characteristic.on('data', (data, isNotification) => {
    operation.receivedSampleBytes += data.length;
